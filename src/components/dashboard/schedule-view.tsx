  ...
  React.useEffect(() => {
    if ((dialogState.mode === 'details' || dialogState.mode === 'edit') && dialogState.event) {
      setEditOrderForm({
        ...
        carName: findKey(dialogState.event.raw, ["車名", "車両", "車種"]) || '',
        regNo: findKey(dialogState.event.raw, ["登録ナンバー(下４桁)", "登録ナンバー", "ナンバー", "車番", "登録番号"]) || '',
        arrivalStatus: findKey(dialogState.event.raw, ["入庫状況"]) || '',
        tireNumber: findKey(dialogState.event.raw, ["タイヤ品番", "品番"]) || '',
        tireSize: findKey(dialogState.event.raw, ["タイヤサイズ", "サイズ", "Size", "タイヤ名/サイズ"]) || '',
        productName: findKey(dialogState.event.raw, ["品名", "商品名"]) || '',
        taskDetails: findKey(dialogState.event.raw, ["作業内容", "業務内容", "taskDetails", "Description", "作業", "作業内容・商品詳細", "内容"]) || '',
        quantity: findKey(dialogState.event.raw, ["本数", "honsu", "数量", "Qty", "Quantity", "本", "タイヤ本数"]) || '',
        sensor: findKey(dialogState.event.raw, ["空気圧センサーパッキン交換", "センサー"]) || '',
        tireStatus: findKey(dialogState.event.raw, ["タイヤ手配状況", "手配"]) || '',
        disposal: findKey(dialogState.event.raw, ["廃タイヤ処分", "廃タイヤ"]) || '',
        specialNotes: findKey(dialogState.event.raw, ["特記事項", "備考", "メモ", "特記", "specialNotes"]) || '',
      ...
      setEditOrderForm({
        ...
        carName: findKey(dialogState.order.raw, ["車名", "車両", "車種"]) || '',
        regNo: findKey(dialogState.order.raw, ["登録ナンバー(下４桁)", "登録ナンバー", "ナンバー", "車番", "登録番号"]) || '',
        arrivalStatus: findKey(dialogState.order.raw, ["入庫状況"]) || '',
        tireNumber: findKey(dialogState.order.raw, ["タイヤ品番", "品番"]) || '',
        tireSize: findKey(dialogState.order.raw, ["タイヤサイズ", "サイズ", "Size", "タイヤ名/サイズ"]) || '',
        productName: findKey(dialogState.order.raw, ["品名", "商品名"]) || '',
        taskDetails: findKey(dialogState.order.raw, ["作業内容", "業務内容", "taskDetails", "Description", "作業", "作業内容・商品詳細", "内容"]) || '',
        quantity: findKey(dialogState.order.raw, ["本数", "honsu", "数量", "Qty", "Quantity", "本", "タイヤ本数"]) || '',
        sensor: findKey(dialogState.order.raw, ["空気圧センサーパッキン交換", "センサー"]) || '',
        tireStatus: findKey(dialogState.order.raw, ["タイヤ手配状況", "手配"]) || '',
        disposal: findKey(dialogState.order.raw, ["廃タイヤ処分", "廃タイヤ"]) || '',
        specialNotes: findKey(dialogState.order.raw, ["特記事項", "備考", "メモ", "特記", "specialNotes"]) || '',
