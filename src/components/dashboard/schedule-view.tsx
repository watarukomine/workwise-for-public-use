...
import { cn, findKey, mapRawToOrder, getContrastingTextColor } from '@/lib/utils';
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if ('staffId' in item) { // Moving an existing event
        ...
        try {
            ...
            refetchOrders(); // Revert UI on error
        }
...
const DraggableEvent: React.FC<DraggableEventProps> = ({ event, staff, getCustomerByCode, onDoubleClick }) => {
  ...
  const textColor = getContrastingTextColor(staff.color || '#000000');
