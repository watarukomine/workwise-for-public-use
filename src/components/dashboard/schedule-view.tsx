  ...
  const handleSendIcs = async (event: WithId<ScheduleEvent>) => {
    ...
    try {
      const result = await sendIcsEmail({
        ...
        title: event.title,
        description: [
          `店舗名: ${findKey(event.raw, ['お取引先名', '店舗']) || event.customerName || 'N/A'}`,
          `住所: ${findKey(event.raw, ['住所']) || event.address || 'N/A'}`,
          `車名: ${findKey(event.raw, ['車名']) || (event as any).carName || 'N/A'}`,
          `登録ナンバー: ${findKey(event.raw, ['登録ナンバー(下４桁)']) || (event as any).regNo || 'N/A'}`,
          `作業内容: ${findKey(event.raw, ['作業内容']) || event.taskDetails || 'N/A'}`,
          `サイズ/本数: ${findKey(event.raw, ['タイヤサイズ']) || (event as any).tireSize || 'N/A'} / ${findKey(event.raw, ['本数']) || (event as any).tireNumber || 'N/A'}`,
          `特記事項: ${findKey(event.raw, ['特記事項', 'specialNotes', 'コメント']) || (event as any).comment || (event as any).specialNotes || 'なし'}`,
          `フォーム入力者: ${event.submitter || 'なし'}`,
        ].join('\n'),
        startTime: event.start as string,
        endTime: event.end as string,
        location: findKey(event.raw, ['住所']) || event.address || '',
        isUpdate: false,
