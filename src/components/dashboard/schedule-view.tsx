  ...
  const handleSendIcs = async (event: WithId<ScheduleEvent>) => {
    ...
    try {
      const result = await sendIcsEmail({
        ...
        title: event.title,
        description: [
          `店舗名: ${findKey(event.raw, ['お取引先名', '店舗']) || 'N/A'}`,
          `住所: ${findKey(event.raw, ['住所']) || 'N/A'}`,
          `車名: ${findKey(event.raw, ['車名']) || 'N/A'}`,
          `登録ナンバー: ${findKey(event.raw, ['登録ナンバー(下４桁)']) || 'N/A'}`,
          `作業内容: ${findKey(event.raw, ['作業内容']) || 'N/A'}`,
          `サイズ/本数: ${findKey(event.raw, ['タイヤサイズ']) || 'N/A'} / ${findKey(event.raw, ['本数']) || 'N/A'}`,
          `特記事項: ${findKey(event.raw, ['特記事項', 'specialNotes']) || 'なし'}`,
